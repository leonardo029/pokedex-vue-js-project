    ...
    >
      <template v-if="teamStore.team[i - 1]">
        ...
        >
          <v-btn
            ...
            @click="removeFromTeam(teamStore.team[i - 1].id)"
          >
      ...
      <template v-else>
        <v-card
          ...
          rounded
        >
          <span class="text-h5 font-weight-bold">Pokémon</span>
...
export default {
  ...
  methods: {
    ...
    removeFromTeam(idPokemon: number) {
      this.teamStore.removePokemon(idPokemon);
    },
  }
};
</script>
